...
import {importSchoolSchoolTypeExcelUsingPost} from '@/services/henu-backend/excelController';
...
const UploadAdminModal: React.FC<Props> = (props) => {
  ...
  return (
    <ModalForm
      title={'批量导入高校与高校类型关联信息'}
      open={visible}
      form={form}
      onFinish={async (values: any) => {
        const hide = message.loading('正在高校与高校类型关联信息，请稍等...');
        try {
          const res = await importSchoolSchoolTypeExcelUsingPost({
